...
ci.tukey(.05, m, sd, n) # statpsych version
ci.tukey(.05, m, sd, n) |> sp.tukey() # Fixes the first column and row names

### CIs for a Set of Sample Means

ci.mean(.05, 5.2, 1.1, 30) # statpsych version
ci.mean(.05, 6.1, 1.3, 28) # statspsych version
ci.mean.vec(alpha = .05, m = c(5.2, 6.1), sd = c(1.1, 1.3), n = c(30, 28)) # Vectorized Version

### Vectorize Two Group Mean Difference

ci.mean2(.05, 15.4, 10.3, 2.67, 2.15, 30, 20) # statpsych version
ci.mean2.vec(.05, c(15.4, 10.3), c(2.67, 2.15), c(30, 20)) # Vectorized Version

ci.stdmean2(.05, 35.1, 26.7, 7.32, 6.98, 30, 30) # statpsych version
ci.stdmean2.vec(.05, c(35.1, 26.7), c(7.32, 6.98), c(30, 30)) # Vectorized Version

### Vectorize Paired Samples Mean Difference

ci.mean.ps(.05, 58.2, 51.4, 7.43, 8.92, .537, 30) # statpsych version
ci.mean.ps.vec(.05, c(58.2, 51.4), c(7.43, 8.92), .537, 30) # Vectorized Version

ci.stdmean.ps(.05, 110.4, 102.1, 15.3, 14.6, .75, 25) # statpsych version
